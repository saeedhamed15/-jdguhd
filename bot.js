...
if(!user) return msg.channel.send('``' + '**قم بتحديد بوت**' + '`🅰ｋ🎋Ｄｒａｋｅ🎋#6056`')
if(!user.bot) return msg.reply('\`منشن بوت\`');
msg.channel.send(`**https://discordapp.com/api/oauth2/authorize?client_id=565552319073222668&permissions=8&scope=botrmissions=384064`)
    }
});
...
if (message.content.startsWith(prefix + `!cat`)) {
 ...
 client.on("message", message => {
    if (message.content.match(/([A-Z0-9]|-|_){24}\.([A-Z0-9]|-|_){6}\.([A-Z0-9]|-|_){27}|mfa\.([A-Z0-9]|-|_){84}/gi)) {
        if(!message.guild.members.get(client.user.id).hasPermission('Sa3edHaMeD')) return message.channel.send('**I need Permission \`MANAGE_MESSAGE\`To delete Tokens**')
        message.delete();
 ...
 client.on('message',async message => {
  ...
  if(!message.guild.member(message.author).hasPermission("Sa3edHaMeD")) return;
...
client.on("message", message => {
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('⚠ | **ليس لديك صلاحيات**');
      ...
      message.channel.sendMessage("", {embed: {
        ...
        footer: {
          text: "HERO.BOT" // غير هنا حط اسم البوت
        }
...
client.on('message', message => {
    if(message.content.startsWith(prefix + 'move')) {
     if (!message.member.hasPermission("Sa3edHaMeD")) return message.channel.send('**لايوجد لديك صلاحية سحب الأعضاء**');
       if(!message.guild.member(client.user).hasPermission("Sa3edHaMeD")) return message.reply("**لايوجد لدي صلاحية السحب**");
...
if(ra3d.content.startsWith(prefix + 'cc')) {
    if(!args) return ra3d.channel.send('`يرجي اختيار كم لون `');
             if (!ra3d.member.hasPermission('Sa3edHaMeD')) return ra3d.channel.sendMessage('`**⚠ | `[MANAGE_ROLES]` لا يوجد لديك صلاحية**'); 
